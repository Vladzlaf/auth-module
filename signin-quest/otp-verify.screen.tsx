...
import { signinQuestComplete } from '../store/auth.actions';
import { isSessionPendingSelector } from '../store/auth.selectors';


const CodeVerificationScreen: FC<any> = ({ route }) => {
  ...
  const signInComplete = async () => {
    ...
    const { payload } = await dispatch(signinQuestComplete(signInCompleteData));
